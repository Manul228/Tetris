...
class AppModel {
    ...
    private fun setCellStatus(row: Int, column: Int, staus: Byte?) {
        if (staus != null) {
            field[row][column] = staus
        }
    }
    ...
    private fun persistCellData() {
        for (i in field.indices) for (j in field[i].indices) {
    ...
    private fun shiftRows(nToRow: Int) {
        if (nToRow > 0) {
            for (j in nToRow - 1 downTo 0) for (m in field[j].indices) {
